...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
    ...
    case SWITCH_ABC_TYPE_WRITE_REPLACE:
    {
        ...
        if (tech_pvt && tech_pvt->playbackBuffer)
        {
            switch_frame_t *rframe = switch_core_media_bug_get_write_replace_frame(bug);
            if (rframe && rframe->data && rframe->datalen)
            {
                size_t want = rframe->datalen;
                switch_mutex_lock(tech_pvt->playbackMutex);
                size_t avail = ringBufferLen(tech_pvt->playbackBuffer);

                /* jitter buffer: chua phat cho toi khi gom du PLAYBACK_PREFILL_BYTES */
                if (!tech_pvt->playbackActive && avail >= PLAYBACK_PREFILL_BYTES)
                {
                    tech_pvt->playbackActive = 1;
                }

                if (tech_pvt->playbackActive && avail >= want)
                {
                    ringBufferGetMultiple(tech_pvt->playbackBuffer, (uint8_t *)rframe->data, want);
                    switch_mutex_unlock(tech_pvt->playbackMutex);
                    switch_core_media_bug_set_write_replace_frame(bug, rframe);
                }
                else
                {
                    /* dang prime, hoac underrun -> de silence di qua, giu nguyen data */
                    if (tech_pvt->playbackActive && avail == 0)
                    {
                        tech_pvt->playbackActive = 0; /* het audio -> re-prime cho luot sau */
                    }
                    switch_mutex_unlock(tech_pvt->playbackMutex);
                }
            }
                                     ...
                                     char *metadata)
{
    ...
    flags |= SMBF_WRITE_REPLACE; /* enable gapless bot-audio injection (output) */
